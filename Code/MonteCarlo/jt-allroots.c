/* jt-allroots.f -- translated by f2c (version 20100827).
...
/* Subroutine */ int rpoly_(double *op, int *degree, double *
	zeror, double *zeroi, bool *fail)
{
    ...
    static float xx, yy;
    static int nm1;
    static float bnd, min__, max__;
    static int cnt;
    static float xxx, base;
    extern /* Subroutine */ int quad_(double *, double *, double *
	    , double *, double *, double *, double *);
    static double temp[101];
    static float cosr, sinr, infin;
    static bool zerok;
...
L60:
    ...
    i__1 = global_1.nn;
    for (i__ = 1; i__ <= i__1; ++i__) {
	x = (r__1 = (float) global_1.p[i__ - 1], dabs(r__1));
	if (x > max__) {
	    max__ = x;
	}
	if (x != 0. && x < min__) {
...
L110:
    i__1 = global_1.nn;
    for (i__ = 1; i__ <= i__1; ++i__) {
	pt[i__ - 1] = (r__1 = (float) global_1.p[i__ - 1], dabs(r__1));
/* L120: */
    }
    pt[global_1.nn - 1] = -pt[global_1.nn - 1];
/* COMPUTE UPPER ESTIMATE OF BOUND */
    x = exp((log(-pt[global_1.nn - 1]) - log(pt[0])) / (float) global_1.n);
    if (pt[global_1.n - 1] == 0.) {
	goto L130;
    }
...
/* AND DO 5 STEPS WITH NO SHIFT */
    ...
    i__1 = global_1.n;
    for (i__ = 2; i__ <= i__1; ++i__) {
	global_1.k[i__ - 1] = (float) (global_1.nn - i__) * global_1.p[i__ - 1]
		 / (float) global_1.n;
/* L190: */
    }
...
/* Subroutine */ int fxshfr_(int *l2, int *nz)
{
    ...
    static float ss, ts, tv, vv, oss, ots, otv, tss, ovv;
    ...
    static bool vpass;
    extern /* Subroutine */ int calcsc_(int *), floatit_(double *, 
	    int *, int *), quadsd_(int *, double *, 
	    double *, double *, double *, double *, 
	    double *), quadit_(double *, double *, int *), 
...
/* Subroutine */ int quadit_(double *uu, double *vv, int *nz)
{
    ...
    static float t, ee;
    static double ui, vi;
    static float mp, zm, omp;
    extern /* Subroutine */ int quad_(double *, double *, double *
	    , double *, double *, double *, double *);
    static int type__;
    static bool tried;
    extern /* Subroutine */ int nextk_(int *), calcsc_(int *), 
	    quadsd_(int *, double *, double *, double *, 
	    double *, double *, double *), newest_(int *, 
	    double *, double *);
    static float relstp;
...
/* EVALUTING P */
    zm = sqrt((r__1 = (float) global_1.v, dabs(r__1)));
    ee = (r__1 = (float) global_1.qp[0], dabs(r__1)) * 2.;
    t = -global_1.szr * global_1.b;
    i__1 = global_1.n;
    for (i__ = 2; i__ <= i__1; ++i__) {
	ee = ee * zm + (r__1 = (float) global_1.qp[i__ - 1], dabs(r__1));
/* L20: */
    }
    ee = ee * zm + (r__1 = (float) global_1.a + t, dabs(r__1));
    ee = (global_1.mre * 5. + global_1.are * 4.) * ee - (global_1.mre * 5. 
	    + global_1.are * 2.) * ((r__2 = (float) global_1.a + t, dabs(r__2)
	    ) + (r__1 = (float) global_1.b, dabs(r__1)) * zm) + global_1.are * 
	    2. * dabs(t);
...
/* Subroutine */ int floatit_(double *sss, int *nz, int *iflag)
{
    /* System generated locals */
    int i__1;
    float r__1;
...
/* P */
    ms = abs(s);
    ee = global_1.mre / (global_1.are + global_1.mre) * (r__1 = (float) 
	    global_1.qp[0], dabs(r__1));
    i__1 = global_1.nn;
    for (i__ = 2; i__ <= i__1; ++i__) {
	ee = ee * ms + (r__1 = (float) global_1.qp[i__ - 1], dabs(r__1));
/* L30: */
    }
/* ITERATION HAS CONVERGED SUFFICIENTLY IF THE */
/* POLYNOMIAL VALUE IS LESS THAN 20 TIMES THIS BOUND */
    if (mp > ((global_1.are + global_1.mre) * ee - global_1.mre * mp) * 20.) 
	    {
	goto L40;
    }
    *nz = 1;
...
/* L60: */
    ...
    if (abs(kv) <= (d__1 = global_1.k[global_1.n - 1], abs(d__1)) * 10. * 
	    global_1.eta) {
	goto L80;
    }
...
/* Subroutine */ int calcsc_(int *type__)
{
    ...
    /* Local variables */
    extern /* Subroutine */ int quadsd_(int *, double *, double *,
	     double *, double *, double *, double *);
...
/* COMPUTED IN CALCSC */
    ...
    if (abs(global_1.a1) > abs(temp) * global_1.eta * 10.) {
	goto L20;
    }
...
/* Subroutine */ int quadsd_(int *nn, double *u, double *v, 
...
/* Subroutine */ int quad_(double *a, double *b1, double *c__, 
